...
- 2026-09-22: **Next.js side shipped** (`061d4369` table+store, `9fe2eb61` persist+id: lines, `482ec20d` replay route, `3abb56e4` client resume+coalesce). `kanban_events` on pg+sqlite, 7-day retention, replay via `Last-Event-ID` / `?lastEventId` / `?since`, verified live: 3 events written with no client attached, reconnect with row-1 id replays the other 2 with matching ids. **Still open for the Axum backend** — table/frame contract frozen in `docs/exec-plans/completed/kanban-debt-triage-2026-09.md` Item 3; the Rust port is mechanical. Front-end dashboards (WIP / lead time) remain in `2026-03-19-kanban-flow-observability-and-control-gaps.md`.
