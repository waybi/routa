...
2. ~~**Next backend (commit 1)**~~ — done in `7ef890cd`.
   - `taskToUpdateAck` / `taskToMoveAck` added; `kanban-tools.ts` returns changed.
   - 3 ack shape tests added (incl. fat-comment fixture asserting `< 1 KB` and no comment leakage); the one echo-shape assertion in "appends update_card comment notes…" rewritten to the ack shape (its persistence assertions via `taskStore.get` were already independent, so no coverage lost).
   - Tool descriptions updated in `mcp-tool-executor.ts` and `routa-mcp-tool-manager.ts`.
3. ~~**Rust backend (commit 2)**~~ — done in `1c9dca58`.
   - `update_card_ack` / `move_card_ack` projections added in `events_kanban.rs`; RPC untouched.
   - `tool_catalog.rs` descriptions updated for both tools.
   - 4 mirror unit tests added (same fat-comment fixture, same field lists, plus a missing-`card` tolerance case).
4. ~~**Validation**~~ — done, see section below.

## Validation Results

| Check | Result |
|---|---|
| `npx vitest run src/core/tools/__tests__/kanban-tools.test.ts src/core/mcp/__tests__/` | **47 passed / 5 files** |
| `npx tsc --noEmit` | **clean (exit 0)** |
| `cargo test -p routa-server` | **193 passed, 0 failed** |
| `cargo clippy -p routa-server --all-targets` | **clean** |
| `entrix run --tier normal` | **blocked by 3 pre-existing gate failures unrelated to this change** (see below) |

### Pre-existing gate failures (not caused by this change)

`entrix run --tier normal` reports `clippy_pass`, `npm_audit_critical`, and `rust_test_pass` as failing. All three were verified to pre-date this work:

- **`rust_test_pass`** — 3 failures in `routa-core` `workflow::specialist::tests::test_load_default_dirs_*`. Reproduced identically at `HEAD~2` (before commit 1) in a clean isolated `git worktree`, so they are independent of this change. This plan touched neither `routa-core` nor specialist loading.
- **`clippy_pass`** — `routa-desktop` build script fails reading `/Users/ouweibing/Desktop/my/routa/target/.../permissions/app/autogenerated/commands/app_hide.toml`. That is a **stale absolute path from before the repository moved** to `topgames_work/`; it is a local build-cache artifact, not a code defect. Likely fixed by clearing `target/` for the desktop crate.
- **`npm_audit_critical`** — 1 critical among 72 advisories in the dependency tree. Neither commit touched `package.json` / `package-lock.json` (last changed in `ff758161`).

Those three gates should be triaged separately; the crates and surfaces this plan changed are green.

### Measured effect (real baseline card `7e93558c`, 10 comments)

| | bytes | ~tokens |
|---|---|---|
| OLD `update_card` echo (`taskToCard` shape) | 60,311 | ~15,077 |
| NEW `update_card` ack | 117 | ~29 |
| **reduction** | **99.81%** | **~15,048 tokens saved per call** |

Repro: fetch `/api/tasks/7e93558c-4332-48ea-8d18-b0a0fc626db7`, serialize the old `taskToCard` field set vs the new ack field set.

## Exit Criteria

- [x] `update_card` / `move_card` MCP results are the documented ack on both backends, byte-shape-identical for the same operation.
- [x] Comment-heavy cards no longer produce multi-KB tool results for these tools (measured: 60,311 B → 117 B).
- [x] All existing gate/error-path tests pass unchanged (193 Rust + 47 Next).
- [ ] `entrix run --tier normal` passes — **blocked by 3 pre-existing failures unrelated to this change** (triage separately; see Validation Results).
- [ ] Runtime probe on a live Kanban agent turn — **not run**: requires rebuilding/restarting the dev server, which was serving pre-change code during this work.
